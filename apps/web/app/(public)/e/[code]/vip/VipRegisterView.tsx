...
/**
 * Shared VIP express registration view. A guest who has the event's shared VIP
 * link lands here, gives name + email only, and is issued a ticket immediately.
 * The event's custom questions are skipped by design.
 *
 * The token is passed in by the route wrapper: the compact link carries it as
 * the last path segment (`/e/<code>/vip/<token>`), and the older query form
 * (`/e/<code>/vip?t=<token>`) still resolves to the same view.
 */
export default function VipRegisterView({ code, token }: { code: string; token: string }) {

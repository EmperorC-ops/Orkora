...
export function ticketConfirmationTemplate(input: {
  ...
  // Shareable card hero for the lead ticket. The card image is generated by the
  // web app's OG route and the URL burned into it drives friends to the event,
  // turning the confirmation email into a growth surface. Skipped if there is
  // somehow no ticket.
  const heroCode = tickets[0]?.code;
  const shareHero = heroCode
    ? `
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td align="center" style="padding:0 0 24px 0;">
        <img src="${APP_URL}/og/ticket/${encodeURIComponent(heroCode)}?format=square" alt="Your shareable card" width="320" style="display:block;width:100%;max-width:320px;border-radius:16px;border:0;outline:none;text-decoration:none;" />
        <div style="margin-top:12px;font-size:15px;color:#0F172A;font-weight:600;">You are in. Share it.</div>
        <a href="${APP_URL}/t/${encodeURIComponent(heroCode)}" style="display:inline-block;margin-top:10px;background:#6D28D9;color:#ffffff;text-decoration:none;padding:10px 18px;border-radius:999px;font-weight:600;font-size:13px;">Share your ticket</a>
      </td></tr>
    </table>`
    : '';
